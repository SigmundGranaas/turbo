...
```
client → tiles.turkart.no (Worker)
...
1. Create the bucket + lifecycle rule (once):
   ...
   ```
2. DNS: `tiles.turkart.no` proxied (orange-cloud) at the `turkart.no` zone.
...
```sh
curl -sI https://tiles.turkart.no/v1/basemap/12/2170/1189.mvt | grep x-tiles-cache
