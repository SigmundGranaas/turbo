...
import 'indexdb_marker_datastore.dart';
import 'marker_data_store.dart';
import 'pending_delete_store.dart';
import 'sqlite_marker_datastore.dart';
...
class LocationRepository extends Notifier<AsyncValue<List<Marker>>> {
  ...
  Future<PendingDeleteStore> get _pendingDeletes => ref.read(pendingDeleteStoreProvider.future);
  ...
  Future<void> _syncLocalDataOnLogin({required AuthStatus authStatus}) async {
    ...
    // Drain pending deletes before uploading new markers
  ...
  Future<List<Marker>> _fetchAndCacheAllUserMarkersFromServer() async {
    ...
    try {
      ...
      if (localStore is SQLiteMarkerDataStore) {
        ...
        // Only clear synced markers to preserve unsynced local changes
        await localStore.db.transaction((txn) async {
          await txn.delete(markersTable, where: 'synced = ?', whereArgs: [1]);
        ...
        // Preserve unsynced markers across server sync
        // Insert server markers first, then unsynced locals last so local-changes-win
        // (IndexedDB put() is upsert — last write wins)
        final unsynced = await localStore.getUnsynced();
        await localStore.clearAll();
        for (final marker in serverMarkers) {
  ...
  Future<void> deleteMarker(String uuid) async {
    ...
    try {
      ...
      _log.warning("Error deleting marker from server, queuing for retry", e, st);
      final deleteStore = await _pendingDeletes;
      await deleteStore.add(uuid);
      await localStore.delete(uuid);
      await _loadData(authStatus: currentAuthStatus);
    }
  }
  ...
  Future<void> _processPendingDeletes() async {
    final deleteStore = await _pendingDeletes;
    final pendingDeletes = await deleteStore.getAll();
    for (final uuid in pendingDeletes) {
      try {
        await _apiService.deleteLocation(uuid);
        await deleteStore.remove(uuid);
      } catch (e) {
        _log.warning("Sync: failed to delete $uuid from server", e);
      }
    }
  }

  /// Uploads all unsynced local markers to the server.
  Future<void> _uploadUnsyncedMarkers() async {
    final localStore = await _localStore;
    final unsynced = await localStore.getUnsynced();
    for (final marker in unsynced) {
      try {
        final serverMarker = await _apiService.createLocation(marker);
        if (serverMarker != null) {
          await localStore.delete(marker.uuid);
        ...
        _log.warning("Sync: failed to upload marker ${marker.uuid}", e);
      }
    }
  }
