...
impl CellForm {
    pub fn refused() -> Self {
        Self {
            norm: NormForm { offsets: [[0; 3]; 3], weights: [0.0; 3], n_terms: 0 },
            rhs: f32::INFINITY,
        }
    }
    pub fn is_refused(&self) -> bool { !self.rhs.is_finite() }
}

impl Default for CellForm {
    fn default() -> Self { Self::refused() }
}
...
pub fn solve_2d_anisotropic(
    ...
    for &(si, sj, u0) in seeds {
        if si >= shape.nx || sj >= shape.ny { continue; }
        let flat = shape.idx(si, sj, 0);
        if forms.flat()[flat].is_refused() { continue; }
    ...
    while let Some((u_a, cell_a)) = heap.pop_min() {
        ...
        if state[flat_a] == NodeState::Accepted { continue; }
        if arrival.flat()[flat_a] < u_a { continue; }
        ...
        for k in 0..form_a.norm.n_terms as usize {
            ...
            for &sign in &[-1i32, 1] {
                ...
                if bi < 0 || bj < 0 || bi >= nx_i || bj >= ny_i { continue; }
                let bflat = shape.idx(bi as u32, bj as u32, 0);
                if state[bflat] == NodeState::Accepted { continue; }
                let form_b = forms.get(bi as u32, bj as u32, 0);
                if form_b.is_refused() { continue; }
                let candidate = anisotropic_update_at(shape, forms, &arrival, &state, bi as u32, bj as u32);
...
fn anisotropic_update_at(
    ...
    if form_b.is_refused() { return f32::INFINITY; }
...
mod tests {
    ...
    fn anisotropic_axis_aligned_faster_along_cheap_direction() {
        ...
        let centre = n / 2;
        let r = solve_2d_anisotropic(shape, &forms, &[(centre, centre, 0.0)], StopCondition::AllAccepted);
        let u_x = r.arrival.get(centre + 10, centre, 0);
        let u_y = r.arrival.get(centre, centre + 10, 0);
        assert!(u_x < u_y, "along-x should arrive sooner; got u_x={u_x}, u_y={u_y}");
