...
CREATE OR REPLACE VIEW paths.v_ski_tracks AS
    SELECT
        ('edge:' || e.id)::text, e.geom,
        NULL::text, NULL::text, NULL::text,
        e.length_m, e.elevation_gain_m, e.elevation_loss_m,
        e.marking, e.surface, e.season,
        'fkb'::text, '© Kartverket'::text
    FROM paths.edge e
    WHERE e.deleted_at IS NULL
      AND e.fkb_type = 'skiloype'
UNION ALL
    SELECT
        ('route:' || r.id::text), ST_GeometryN(r.geom, 1),
        r.name, r.description, r.difficulty,
        r.length_m, r.elevation_gain_m, r.elevation_loss_m,
        r.marking, r.surface, r.season,
        r.source, COALESCE(r.attribution, '© Kartverket, Skisporet.no')
    FROM paths.curated_route r
...
UNION ALL
    SELECT
        ...
        r.length_m, r.elevation_gain_m, r.elevation_loss_m,
        r.marking, r.surface, r.season,
        r.source, COALESCE(r.attribution, '© Kartverket')
    FROM paths.curated_route r
