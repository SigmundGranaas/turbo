...
public sealed class PackraftingRecommendationScorer : IActivityRecommendationScorer
{
    ...
    private readonly ILogger<PackraftingRecommendationScorer> _logger;

    public PackraftingRecommendationScorer(
        IPackraftingActivityReader reader,
        PackraftingOrchestrator orchestrator,
        IActivityGeoContextService geoContext,
        ILogger<PackraftingRecommendationScorer> logger)
    {
        _reader = reader;
    ...
    public string Kind => "packrafting";

    private static readonly HashSet<string> CheapKeys = new(StringComparer.Ordinal)
    {
        PackraftingOrchestrator.ProviderKeyWeather,
        PackraftingOrchestrator.ProviderKeyRiverFlow,
    };
        ...
        IReadOnlyList<Guid> activityIds, QueryContext queryContext, CancellationToken cancellationToken)
    {
        ...
        foreach (var id in activityIds)
        {
            ...
            {
                _logger.LogWarning(ex, "Packrafting quick-score failed for {Id}", id);
            }
        }
        return results;
    }
