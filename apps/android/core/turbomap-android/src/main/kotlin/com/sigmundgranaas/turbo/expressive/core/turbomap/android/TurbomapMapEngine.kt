...
class TurbomapMapEngine(
    ...
    override fun flyTo(target: LatLng, zoom: Double) {
        val bearing = camera().getOrElse(3) { 0.0 }
        setCamera(target.lat, target.lng, zoom, bearing)
    }
    ...
    // The live sheet inset will reposition the camera target once the on-screen
    // host is wired (Stage E remainder); harmless no-op until then.
    override fun setBottomInset(bottomPx: Int) = Unit
